...
gate() {
  ...
  if load_conf; then
    ...
    say "           (a PROJECTION of the policy graph — if the graph has"
    say "            gained a pattern since, this gate is behind it)"
    say "           REGENERATE FROM THE PATH BELOW, WHICH NOBODY OWNS:"
    say "             cd ~/.local/lib/aegis-cd/goldblum \\"
    say "               && python3 policy/emit-scrub-config.py --stdout"
    say "           then run the pre-push guard's --selftest afterwards."
    say "           That checkout is swept by aegis-cd and held READ-ONLY"
    say "           between sweeps, so it cannot be edited in place, and"
    say "           check-aegis-cd-goldblum-freshness.sh pages if the sweep"
    say "           stops (aegis-5grao). Do NOT use ~/workspace/goldblum or"
    say "           ~/gt/goldblum: those are mutable personal/shared trees."
    say "           WHY A NAMED PATH: a stale copy of that emitter writes a"
    say "           config whose private-IP arm matches NOTHING under grep -E,"
    say "           and it exits 0 saying it wrote the file. Measured"
    say "           2026-08-30: this line said only \"goldblum\", the obvious"
    say "           checkout was 426 commits behind, and following this very"
    say "           instruction disabled an arm of the live guard."
  else
