...
// The column classification — every table quipu creates, and whether each
// column carries term identity — lives in `respace_map` (size ratchet split);
// the public paths stay `respace::TermIdKind` / `respace::COLUMN_CLASSIFICATION`.
pub use super::respace_map::{COLUMN_CLASSIFICATION, TermIdKind};
