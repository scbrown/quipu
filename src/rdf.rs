...
pub(crate) fn serialize_facts(
    ...
    serialize_triples_canonical(&triples, format)
}

/// Serialize a graph in a stable order independent of term-interning order.
///
/// N-Triples is the canonical intermediate because each statement occupies one
/// line. Sorting and deduplicating those lines makes equal IRI-only graphs
/// byte-identical even when stores assigned different integer term ids. Blank
/// node identifiers are preserved here; the share-bundle layer applies RDF
/// Dataset Canonicalization before using these bytes as a cross-store hash.
fn serialize_triples_canonical(triples: &[Triple], format: RdfFormat) -> Result<Vec<u8>> {
...
/// Export the ROOT facts attributed to one episode `group_id`.
///
/// Group ids are provenance labels, not graph boundaries. The slice therefore
/// contains each matching episode resource and every current ROOT fact whose
/// subject declares `prov:wasGeneratedBy` that episode. It deliberately does
/// not pull unrelated objects recursively: callers wanting a graph-shaped lens
/// should use a CONSTRUCT export instead.
pub fn export_rdf_group(
    ...
    let escaped = group_id
        ...
        .replace('\r', "\\r");
    let query = format!(
        "SELECT DISTINCT ?s ?ep WHERE {{ ?ep <{}groupId> \"{escaped}\" . \
         ?s <{}wasGeneratedBy> ?ep . }}",
        store.base_ns(),
        namespace::PROV,
    );
...
/// Export the graph returned by a SPARQL CONSTRUCT query.
...
"#;
    ...
    fn canonical_export_is_independent_of_interning_order() {
        ...
        assert_eq!(l, r, "equal graphs must not inherit store term-id order");
        assert!(
            String::from_utf8(l)
                .unwrap()
    ...
    fn group_export_includes_only_attributed_entities_and_episode() {
        ...
        let data = br#"@prefix ex: <http://example.org/> .
@prefix prov: <http://www.w3.org/ns/prov#> .
ex:ep1 ex:groupId "wanted" .
ex:ep2 ex:groupId "other" .
ex:alice prov:wasGeneratedBy ex:ep1 ; ex:name "Alice" .
ex:bob prov:wasGeneratedBy ex:ep2 ; ex:name "Bob" .
"#;
        ingest_rdf(
            &mut store,
            &data[..],
        ...
        assert!(text.contains("http://example.org/ep1"));
        assert!(!text.contains("http://example.org/bob"));
        assert!(!text.contains("http://example.org/ep2"));
    }
    ...
    fn construct_export_requires_a_graph_result_and_is_sorted() {
        ...
        ingest_rdf(
            &mut store,
            &b"<http://example.org/z> <http://example.org/name> \"Z\" .\n<http://example.org/a> <http://example.org/name> \"A\" .\n"[..],
            RdfFormat::NTriples,
            None,
            "2026-01-01",
            None,
