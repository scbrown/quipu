...
mod ingest;
pub use ingest::{
    INGEST_NS, IngestReport, LoadDeclaration, ingest_rdf_chunked, ingest_rdf_chunked_with_scope,
    ingest_rdf_declared, ingest_rdf_declared_with_scope,
};
...
pub fn ingest_rdf_bitemporal(
    ...
    ingest_rdf_bitemporal_with_scope(
        store, reader, format, base_iri, valid_from, timestamp, actor, source, graph, None,
    )
}

/// Ingest with an explicit blank-node document scope. Same input and scope share nodes across graphs.
#[allow(clippy::too_many_arguments)]
pub fn ingest_rdf_bitemporal_with_scope(
    store: &mut Store,
    reader: impl Read,
    format: RdfFormat,
    base_iri: Option<&str>,
...
pub(crate) fn parse_rdf_scoped(
    ...
    let (reader, scope) = crate::rdf_scope::prepare(store, reader, graph, blank_node_scope)?;
    parse_rdf_inner(store, reader, format, base_iri, valid_from, Some(scope))
}

/// Preserve assigned transport identities when importing verified shares.
#[allow(clippy::too_many_arguments)]
pub(crate) fn ingest_assigned_rdf(
    store: &mut Store,
    reader: impl Read,
    format: RdfFormat,
    base_iri: Option<&str>,
    ...
    let count = datums.len();
    let tx = store.transact_to_graph(&datums, timestamp, actor, source, graph)?;
    Ok((tx, count))
}

/// Parse transport facts whose blank-node identities are already scoped.
/// Used for internal share deltas and generated episode documents, not new loads.
pub(crate) fn parse_rdf(
    ...
    parse_rdf_inner(store, reader, format, base_iri, valid_from, None)
}
...
fn parse_rdf_inner(
    ...
    base_iri: Option<&str>,
    valid_from: &str,
    mut scope: Option<crate::rdf_scope::Scope>,
) -> Result<Vec<Datum>> {
    ...
    for result in parser.for_reader(reader) {
        ...
        let triple = match &mut scope {
            Some(scope) => scope.triple(triple),
            None => triple,
        };
