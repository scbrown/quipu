...
> **This comparison is a self-assessment, not a measurement.** Some cells are
> contestable (Jena, for example, ships a rule reasoner). For measured numbers,
> see [SPARQL 1.1 conformance](benchmarks/conformance.md), where quipu and other
> stores are scored by the same harness.
...
[Named Graphs](concepts/named-graphs.md) for the registration,
...
| Named graphs (`GRAPH`/`FROM`/`FROM NAMED`) | ✅ | Query side; writes go via overlays or `/episode`. See [named-graphs.md](https://github.com/scbrown/quipu/blob/main/docs/design/named-graphs.md) |
...
| **Governance ([SARC](reference/cli.md#quipu-audit-tracejsonl) conformance)** | | |
