...
/// Execute a temporal SPARQL query (with optional valid_at / as_of_tx).
pub async fn temporal_query(
    ...
    if let Some(vt) = valid_at {
        body["valid_at"] = json!(vt);
    }
    if let Some(tx) = as_of_tx {
        body["as_of_tx"] = json!(tx);
    }
    let resp = Request::post(&url)
        .json(&body)
        .map_err(|e| format!("Request build error: {e}"))?
...
/// Fetch temporal graph — same as fetch_initial_graph but with time context.
pub async fn fetch_temporal_graph(
    valid_at: Option<&str>,
    as_of_tx: Option<i64>,
) -> Result<(Vec<EntityNode>, Vec<(String, String, String)>), String> {
    ...
    if !resp.ok() {
        // Fallback to sparql-only query
        return fetch_initial_graph().await;
    }
...
/// Fetch entity history — all assertions/retractions for an entity.
pub async fn fetch_entity_history(iri: &str) -> Result<Value, String> {
    let url = format!("{}/entity_history", base_url());
    let resp = Request::post(&url)
        .json(&json!({ "iri": iri }))
        .map_err(|e| format!("Request build error: {e}"))?
        .send()
...
/// Fetch all transactions (for time slider range).
pub async fn fetch_transactions() -> Result<Value, String> {
...
/// Fetch episodes via SPARQL (prov:Activity entities).
pub async fn fetch_episodes() -> Result<Value, String> {
    let query = r#"SELECT ?ep ?label ?source ?comment WHERE {
    ...
    sparql_query(query).await
}

/// Fetch entities generated by a specific episode.
pub async fn fetch_episode_entities(episode_iri: &str) -> Result<Value, String> {
    let query = format!(
        r#"SELECT ?s ?p ?o WHERE {{
            ?s <http://www.w3.org/ns/prov#wasGeneratedBy> <{episode_iri}> .
            ?s ?p ?o .
        }}"#
    );
    sparql_query(&query).await
